...
export default function GameCard({ game }: { game: GameRow }) {
  ...
  // On desktop, tags are always visible (existing behavior).
  // On mobile, tags only show once expanded.
  const showTags = !isMobile || expanded;
  ...
  return (
    ...
    >
      ...
      {/* bottom overlay — shrinks to just enough for the title strip when collapsed on mobile */}
      <div
        className={[
          'absolute inset-x-0 bottom-0 bg-linear-to-t from-black/90 via-black/35 to-transparent transition-all duration-300',
          isMobile && !expanded ? 'h-1/3' : 'inset-0',
        ].join(' ')}
      />
      ...
      <div
        className={[
          'absolute inset-x-0 bottom-0 flex flex-col gap-1.5 p-2.5 sm:gap-2 sm:p-4',
      ...
      >
        <div className='flex items-center justify-between gap-2'>
          ...
          {/* Tap hint, mobile only, hides once expanded */}
          {isMobile && !expanded && tags.length > 0 && (
            <span className='shrink-0 text-[10px] text-white/60'>tap</span>
          )}
        </div>
        ...
        {tags.length > 0 && (
          <div
            className={[
              'flex min-w-0 flex-nowrap items-center gap-1 overflow-hidden transition-all duration-300 ease-out',
              isMobile
                ? expanded
                  ? 'max-h-8 opacity-100'
                  : 'max-h-0 opacity-0'

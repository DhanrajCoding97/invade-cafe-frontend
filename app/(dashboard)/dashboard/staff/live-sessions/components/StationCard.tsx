...
export function StationCard({
  ...
  // Flat HUD palette — border/glow carries the status, no extra chrome.
  const hud = {
    active: {
      shell:
        'border-cyan-400/60 bg-[#04080f] shadow-[0_0_0_1px_rgba(34,211,238,0.15),0_0_28px_-8px_rgba(34,211,238,0.55)]',
      chip: 'bg-cyan-400 text-black shadow-[0_0_14px_-2px_rgba(34,211,238,0.9)]',
      accent: 'text-cyan-300',
      bar: 'bg-cyan-400 shadow-[0_0_10px_1px_rgba(34,211,238,0.9)]',
    },
    due: {
      shell:
        'border-amber-400/60 bg-[#0d0902] shadow-[0_0_28px_-10px_rgba(251,191,36,0.6)]',
  ...
  return (
    <div
      className={`rounded-xl border p-3 transition-all duration-300 sm:p-3.5 ${hud.shell}`}
    >
      ...
      {booking && (
        <>
          {/* identity + time row — countdown sits inline when active */}
          ...
          <div className='mt-3 flex items-center gap-2'>
            {isBooked && bookingStatus !== 'no-show' && (
              <Button
                onClick={() => onStart(booking)}
                disabled={isPending}
                className='flex flex-1 items-center justify-center gap-1.5 rounded-lg bg-cyan-400 px-3 py-1.5 font-mono text-xs uppercase tracking-wider text-black shadow-[0_0_18px_-4px_rgba(34,211,238,0.9)] hover:bg-cyan-300 disabled:opacity-60'
            ...
            {bookingStatus === 'no-show' && (
              <AlertDialog>
                <AlertDialogTrigger asChild>
                  <button className='flex-1 rounded-lg border border-red-500/30 bg-red-500/10 px-3 py-1.5 font-mono text-xs uppercase tracking-wider text-red-300 transition-colors hover:bg-red-500/20'>
                    No Show
                  </button>
                </AlertDialogTrigger>
                ...
                <AlertDialogContent size='sm'>
                  <AlertDialogHeader>
                    <AlertDialogTitle>Mark as No Show?</AlertDialogTitle>
                    <AlertDialogDescription>
                      The customer did not arrive within the grace period. This
                      booking will be marked as <strong>No Show</strong>. No
              ...
              <>
                ...
                <AlertDialog>
                  <AlertDialogTrigger asChild>
                    <button
                      disabled={isPending}
                      className='flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-cyan-400/30 bg-cyan-400/10 px-3 py-1.5 font-mono text-xs uppercase tracking-wider text-cyan-200 transition-colors hover:bg-cyan-400/20 disabled:opacity-60'
                    >
                      {isPending && (
                        <Loader2 className='h-3 w-3 animate-spin' />
                      )}

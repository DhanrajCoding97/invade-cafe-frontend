...
//     .from('bookings')
//     .select('date, start_time, duration_hours, extended_until')
//     .eq('id', bookingId)
//     .single();

//   if (!booking) throw new Error('Booking not found');

//   const scheduledEnd = new Date(`${booking.date}T${booking.start_time}`);
//   scheduledEnd.setHours(
//     scheduledEnd.getHours() + Number(booking.duration_hours),
//   );
...
export async function extendSession(
  ...
  // conflict check stays the same — still compares against OTHER bookings'
  // scheduled start times, since those customers' reservations are fixed
  const { data: conflicts } = await supabase
    .from('bookings')
    .select('id, date, start_time')
    .eq('station_id', stationId)
